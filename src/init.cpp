...
#include "config/chavezcoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("chavezcoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "chavezcoin (or specifically: privatesend, instantsend, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Chavezcoin specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N CHAVEZCOIN anonymized (default: %u)"), DEFAULT_PRIVATESEND_AMOUNT));
    ...
    strUsage += HelpMessageOpt("-instantsenddepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-47047, default: %u)"), DEFAULT_INSTANTSEND_DEPTH));
...
/** Sanity checks
 *  Ensure that Chavezcoin Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (nLiqProvTmp > 0) {
        ...
        mapArgs["-privatesendrounds"] = "470479";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendrounds=470479\n", __func__, nLiqProvTmp);
        mapArgs["-privatesendamount"] = "4704799";
        LogPrintf("%s: parameter interaction: -liquidityprovider=%d -> setting -privatesendamount=4704799\n", __func__, nLiqProvTmp);
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Chavezcoin Core version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize Chavezcoin Core.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Chavezcoin Core is shutting down."));
...
#endif
    // Make sure only a single Chavezcoin Core process is using the data directory.
    ...
    try {
        ...
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Chavezcoin Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Chavezcoin Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Chavezcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Chavezcoin Core to complete") << "\n";
    ...
    nPrivateSendRounds = std::min(std::max(nPrivateSendRounds, 2), nLiquidityProvider ? 470479 : 16);
    nPrivateSendAmount = GetArg("-privatesendamount", DEFAULT_PRIVATESEND_AMOUNT);
    nPrivateSendAmount = std::min(std::max(nPrivateSendAmount, 2), 4704799);
    ...
    // ********************************************************* Step 11c: update block tip in Chavezcoin modules
    ...
    // ********************************************************* Step 11d: start chavezcoin-privatesend thread
