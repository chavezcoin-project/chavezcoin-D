...
const char * const BITCOIN_CONF_FILENAME = "chavezcoin.conf";
const char * const BITCOIN_PID_FILENAME = "chavezcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "chavezcoin" is a composite category enabling all Chavezcoin-related debug output
            if(ptrCategory->count(string("chavezcoin"))) {
...
#else
    const char* pszModule = "chavezcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ChavezcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ChavezcoinCore
    // Mac: ~/Library/Application Support/ChavezcoinCore
    // Unix: ~/.chavezcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ChavezcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ChavezcoinCore";
#else
    // Unix
    return pathRet / ".chavezcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty chavezcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override chavezcoin.conf
